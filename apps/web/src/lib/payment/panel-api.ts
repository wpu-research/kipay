 ...
 * apps/gateway/src/services/panel-api.ts'den apps/web'e taşındı.
...
interface PaymentStore {
  ...
  creds:    Map<string, TxCreds>;
  accIdx:   number;
}
const g = globalThis as unknown as { __kipayPayment?: PaymentStore };
const store: PaymentStore = (g.__kipayPayment ??= {
  txs:    new Map<string, Transaction>(),
  creds:  new Map<string, TxCreds>(),
  accIdx: 0,
});
const mockTxMap = store.txs;
...
function makeHeaders(bodyRaw: string, keyId?: string, secret?: string, merchantId?: string) {
  const _keyId      = keyId      || paymentEnv.API_KEY_ID;
  const _secret     = secret     || paymentEnv.API_SECRET;
  const _merchantId = merchantId || paymentEnv.MERCHANT_ID;
...
async function merchantRequest(
  ...
  const url     = `${paymentEnv.PANEL_BASE_URL.replace(/\/$/, '')}${path}`;
...
function useMock(): boolean {
  const url = paymentEnv.PANEL_BASE_URL;
...
function mockDepositInitiate(
  ...
  const acc    = MOCK_ACCOUNTS[store.accIdx % MOCK_ACCOUNTS.length]!;
  store.accIdx++;
